...
public class CreateModel {
	...
	/*
	 * constructor for memory model; namespace is the ontology namespace,
	 * filepath is the path to the ontology
	 */
	...
	public Vector<String> getSubclasses(String cls) {// , String NS){

		OntClass refClass = model.getOntClass(NS + cls);
		Vector<String> classArray = new Vector<String>();
		for (Iterator<OntClass> i = refClass.listSubClasses(); i.hasNext();) {
			OntClass subClass = i.next();
			/*
			 * class Nothing should not be included, even if generated by the
			 * reasoner
			 */
	...
	/*
	 * returns a Vector of direct subclasses of a certain class public Vector
	 * getDirectSubclasses(String cls, String NS){
	 * 
	 * OntClass refClass = model.getOntClass(NS + cls); Vector classArray=new
	 * Vector(); for (Iterator i = refClass.listSubClasses(true); i.hasNext(); ) {
	 * OntClass subClass = (OntClass) i.next(); if
	 * (!(subClass.getLocalName().equals("Nothing"))){
	 * classArray.add(subClass.getLocalName()); } } return classArray; }
	 * 
	 * returns a Vector of direct subclasses of a certain class public Vector<String>
	 * getDirectSubclasses(String cls){//, String NS){
	 * 
	 * OntClass refClass = model.getOntClass(NS + cls); Vector<String>
	 * classArray=new Vector<String>(); for (Iterator i =
	 * refClass.listSubClasses(true); i.hasNext(); ) { OntClass subClass =
	 * (OntClass) i.next(); if (!(subClass.getLocalName().equals("Nothing"))){
	 * System.out.println("Found subclass "+ subClass.getLocalName());
	 * classArray.add(subClass.getLocalName()); } } return classArray; }
	 */

	/* returns a Vector of Individuals of a certain class */
	@SuppressWarnings("unchecked")
	public Vector<String> getIndividuals(String cls) {// ,String NS){

		OntClass refClass = model.getOntClass(NS + cls);
		Vector<String> classArray = new Vector<String>();
		for (Iterator<Individual> i = refClass.listInstances(); i.hasNext();) {
	...
	/* returns a a property value for an Individual given by its instanceName */
	public String getIndividualPropertyValue(String ind, String prop) {// ,String
																		// NS){
		Individual i = getIndividualFromURI(ind, NS);
		String uri = NS + prop;
		...
		if (val != null) {
			if (val instanceof Literal) {
				lVal = (Literal) val;
				System.out.println("Literal - SmartHouse Property" + lVal.getString());
				return (lVal.getString());
			} else {
				OntResource rVal = (OntResource) val.as(OntResource.class);
				System.out.println("Resource - SmartHouse Element " + rVal.getLocalName());
				return (rVal.getLocalName());
			}
		} else {
			System.out.println("NULL- IndividualPropertyValue");
	...
	public Individual createIndividual(String individualName,
			String individualClass, String NS) {
	...
	public void addPropertyValue(String propertyLiteral, String propertyName,
			String individualName, String NS) {
	...
	public void setPropertyValue(String individualName, String property,
			String propertyValue, boolean isLiteral, String NS) {
	...
	/* isLiteral indicates weather the property is a Literal or Resource */
	public void setPropertyValue(String individualName, String property,
			String propertyValue, boolean isLiteral) {
		// System.out.println("TYPED LITERAL-setPropertyValue \n\n"+isLiteral);
		// System.out.println("indivName "+individualName);
		Individual ind = model.getIndividual(NS + individualName);
		Property prop = model.getProperty(NS + property);
	...
	public String getIndividualType(String individual) {

		StringBuffer sb = new StringBuffer();

		for (int i = 0; i < individual.length(); i++) {
			if (Character.isLetter(individual.charAt(i)))
